...
struct Expr {
    ...
    std::variant<Symbol, String, Sig, LambdaPtr, Proc, List> v;

    Expr() = default;
    explicit Expr(const Symbol& x) : v(x) {}
    explicit Expr(const String& x) : v(x) {}
    explicit Expr(const Sig& x) : v(x) {}
...
static ExprPtr make_sig(const Sig& x) { return std::make_shared<Expr>(x); }
static ExprPtr make_scalar(double x) { return make_sig(Sig(x, 1)); }
...
static bool is_sig(const ExprPtr& x) { return std::holds_alternative<Sig>(x->v); }
...
static Sig broadcast(const Sig& a, std::size_t n) {
...
static Sig binop(const Sig& a, const Sig& b, const std::function<double(double, double)>& f) {
    std::size_t n = std::max<std::size_t>(a.size(), b.size());
    Sig x = broadcast(a, n), y = broadcast(b, n), r(n);
    for (std::size_t i = 0; i < n; ++i) r[i] = f(x[i], y[i]);
    return r;
}

static Sig unop(const Sig& a, const std::function<double(double)>& f) {
    Sig r(a.size());
    for (std::size_t i = 0; i < a.size(); ++i) r[i] = f(a[i]);
    return r;
}

static void write_wav_mono(const std::string& path, const Sig& x, int sr) {
    ...
    f.write("RIFF", 4); f.write(reinterpret_cast<char*>(&rs), 4);
    f.write("WAVE", 4); f.write("fmt ", 4); f.write(reinterpret_cast<char*>(&sub), 4);
    f.write(reinterpret_cast<char*>(&fmt), 2); f.write(reinterpret_cast<char*>(&ch), 2);
    f.write(reinterpret_cast<char*>(&sr), 4); f.write(reinterpret_cast<char*>(&br), 4);
    f.write(reinterpret_cast<char*>(&ba), 2); f.write(reinterpret_cast<char*>(&bps), 2);
...
static void write_wav_stereo(const std::string& path, const Sig& l, const Sig& r, int sr) {
...
static ExprPtr apply_callable(const ExprPtr& fn, const std::vector<ExprPtr>& args, std::shared_ptr<Env> env) {
    ...
    if (lam->params.size() != args.size()) throw std::runtime_error("arity mismatch");
    ...
    while (true) {
        ...
        if (is_symbol(xs[0])) {
            ...
            if (head == "sig") {
                ...
                auto local = std::make_shared<Env>();
                local->parent = env;
                local->set("t", make_sig(t));
                local->set("ph", make_sig(ph));
...
static Proc make_numeric_reducer(const std::function<double(double,double)>& f, double init, bool unary_from_init = false) {
    return [f, init, unary_from_init](const std::vector<ExprPtr>& args, std::shared_ptr<Env>) -> ExprPtr {
...
static Proc make_numeric_unary(const std::function<double(double)>& f) {
    return [f](const std::vector<ExprPtr>& args, std::shared_ptr<Env>) -> ExprPtr {
        ...
        return make_sig(unop(as_sig(args[0]), f));
    };
}
...
static Proc fn_cat() {
    return [](const std::vector<ExprPtr>& args, std::shared_ptr<Env>) -> ExprPtr {
        ...
        Sig a = as_sig(args[0]), b = as_sig(args[1]);
        Sig r(a.size() + b.size());
...
static Proc fn_render() {
    return [](const std::vector<ExprPtr>& args, std::shared_ptr<Env> env) -> ExprPtr {
        ...
        if (args.size() - start == 1) {
            Sig x = as_sig(args[start]);
            write_wav_mono(path, x, sr);
            return args[start];
        }
        if (args.size() - start == 2) {
            Sig l = as_sig(args[start]), r = as_sig(args[start + 1]);
...
static std::shared_ptr<Env> make_environment() {
    ...
    env->set("+", make_proc(make_numeric_reducer([](double a, double b) { return a + b; }, 0.0)));
    env->set("-", make_proc(make_numeric_reducer([](double a, double b) { return a - b; }, 0.0, true)));
    env->set("*", make_proc(make_numeric_reducer([](double a, double b) { return a * b; }, 1.0)));
    env->set("/", make_proc(make_numeric_reducer([](double a, double b) { return a / b; }, 1.0, true)));
    env->set("pow", make_proc(make_numeric_reducer([](double a, double b) { return std::pow(a, b); }, 1.0)));

    env->set("sin", make_proc(make_numeric_unary([](double x) { return std::sin(x); })));
    env->set("cos", make_proc(make_numeric_unary([](double x) { return std::cos(x); })));
    env->set("exp", make_proc(make_numeric_unary([](double x) { return std::exp(x); })));
    env->set("abs", make_proc(make_numeric_unary([](double x) { return std::fabs(x); })));
    env->set("sqrt", make_proc(make_numeric_unary([](double x) { return std::sqrt(x); })));
...
int main(int argc, char** argv) {
    ...
    try {
        if (argc > 1) {
            ...
            std::cout << "[pure, v0.1]\n";
            std::cout << "minimalist scheme dialect for sound synthesis\n";
            std::string line;
