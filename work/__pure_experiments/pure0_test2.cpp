...
struct VM {
    ...
    void add_math() {
        ...
        dict["sin"] = B([](VM& vm) {
            vm.push(un(vm.pop_sig(), [](double x) { return std::sin(x); }));
        });
        dict["cos"] = B([](VM& vm) {
            vm.push(un(vm.pop_sig(), [](double x) { return std::cos(x); }));
        });
        dict["exp"] = B([](VM& vm) {
            vm.push(un(vm.pop_sig(), [](double x) { return std::exp(x); }));
        });
        dict["sqrt"] = B([](VM& vm) {
            vm.push(un(vm.pop_sig(), [](double x) { return std::sqrt(x); }));
        });
        dict["abs"] = B([](VM& vm) {
            vm.push(un(vm.pop_sig(), [](double x) { return std::fabs(x); }));
        });
        dict["pow"] = B([](VM& vm) {
            Vector b = vm.pop_sig(), a = vm.pop_sig();
        ...
        dict["pi"] = B([](VM& vm) {
            vm.push(3.14159265358979323846);
        });
    }
        ...
        for (size_t i = 0; i < t.size();) {
            ...
            if (s == "{") {
                ...
                for (size_t k = 0; k < n; ++k) {
                    VM in = *this;
                    in.st.clear();
                    in.dict["t"] = Word{false, {}, {to_string((double)k / SR)}};
                    in.dict["ph"] = Word{false, {}, {to_string(n == 1 ? 0.0 : (double)k / (n - 1))}};
                    in.dict["amp"] = Word{false, {}, {to_string(amp)}};
                    in.exec(body);
                    out[k] = in.pop_num();
                }
                push(out);
