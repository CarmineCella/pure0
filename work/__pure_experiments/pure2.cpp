...
static Vector binop(const Vector& a, const Vector& b, const std::function<double(double, double)>& f) {
...
static void write_wav_stereo(const std::string& path, const Vector& l, const Vector& r, int sr) {
...
static Proc prim_delay() {
    return [](const std::vector<ExprPtr>& args, std::shared_ptr<Env> env) -> ExprPtr {
        ...
        std::size_t n0 = std::max<std::size_t>(0, sec * sr);
        Vector r(x.size() + n0);
...
static std::shared_ptr<Env> make_environment() {
    ...
    env->set("print", make_proc(prim_print()));
    env->set("read", make_proc(prim_read()));
    env->set("write", make_proc(prim_write()));
    env->set("load", make_proc(prim_load()));
    env->set("osc", make_proc(prim_osc()));
    env->set("delay", make_proc(prim_delay()));
    env->set("cat", make_proc(prim_cat()));
    env->set("bpf", make_proc(prim_bpf()));
    env->set("render", make_proc(prim_render()));
